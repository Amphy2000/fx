...
export default function PartnerChatList({
  ...
  return <div className="space-y-3">
      ...
      return <Card key={partnership.id} className="border-border/50 hover:border-primary/50 transition-all cursor-pointer group" onClick={() => handlePartnerClick(partnership.id)}>
            <CardContent className="p-4">
              <div className="flex items-center gap-4">
                <AvatarImage avatarUrl={partnerProfile?.avatar_url} fallbackText={partnerName} className="h-12 w-12" />
                
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-semibold text-lg truncate">{partnerName}</h3>
                    {unreadCount > 0 && <Badge variant="destructive" className="h-5 min-w-[20px] px-1.5 flex items-center justify-center rounded-full text-xs">
