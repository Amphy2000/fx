...
export default function Leaderboard() {
  ...
  const fetchLeaderboard = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('leaderboard_profiles').select('*').eq('is_public', true).order('win_rate', {
        ascending: false
      }).limit(50);
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('leaderboard_profiles').select('*').eq('user_id', user.id).single();
      if (data) {
        setUserProfile(data);
  ...
  const togglePublicProfile = async (checked: boolean) => {
    try {
      ...
      const {
        error
      } = await supabase.from('leaderboard_profiles').upsert({
        user_id: user.id,
        is_public: checked,
  ...
  return <Layout>
      <div className="container mx-auto p-6 max-w-6xl space-y-6">
        ...
        {userProfile && <Card className="border-primary">
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Switch id="public-toggle" checked={isPublic} onCheckedChange={togglePublicProfile} />
              ...
              <CardDescription>
                {isPublic ? "Your stats are visible to other traders" : "Make your profile public to appear on the leaderboard"}
              </CardDescription>
            </CardHeader>
        ...
        <Card>
          ...
          <CardContent>
            {loading ? <div className="text-center py-8 text-muted-foreground">Loading leaderboard...</div> : profiles.length === 0 ? <div className="text-center py-8 space-y-3">
                ...
                {profiles.map((profile, index) => <div key={profile.id} className="flex items-center justify-between p-4 rounded-lg border hover:bg-accent/50 transition-colors">
